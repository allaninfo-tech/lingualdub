...
class TestM8RunyankoleAudit:
    ...
    def test_language_profile_updated(self):
        ...
        assert "2026-09-10" in RUNYANKOLE.resource_profile or "audit" in RUNYANKOLE.resource_profile.lower()
    ...
    def test_resources_registered_with_provenance(self):
        ...
        assert "SALT_ASR_EVAL_PROTOCOL" in RUNYANKOLE_ASR_EVAL_SET.provenance.get("evaluation_protocol", "")
        assert "institutional_open_research_release" in RUNYANKOLE_ASR_EVAL_SET.provenance.get("consent_basis", "")
    ...
    def test_manifest_entries_exist(self):
        import json
        manifest = json.loads(Path("lingualdub/lingualdub.manifest.json").read_text(encoding="utf-8"))
...
class TestM8RunyankoleASRComponent:
    ...
    def test_registered_via_manifest_and_cli(self):
        ...
        assert resolved is RunyankoleASRComponent or issubclass(resolved, RunyankoleASRComponent) or resolved == RunyankoleASRComponent
        # Also check version
    ...
    def test_offline_deterministic_output(self, tmp_path):
        ...
        res = Resource(id="sample_nyn", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "research"})
        result = comp.run(res)
...
class TestM8RunyankoleEvaluation:
    def test_wer_evaluator_on_runyankole_output(self):
        ...
        comp = RunyankoleASRComponent(default_text=RUNYANKOLE_ASR_EVAL_SET.metadata["samples"][0]["reference_text"], use_neural=False)
        res = Resource(id="nyn_sample", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "research", "dataset_version": "1.0.0"})
        result = comp.run(res)
    ...
    def test_wer_nonzero_on_mismatch(self):
        ...
        res = Resource(id="nyn_sample", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "research"})
        result = comp.run(res)
        evaluator = WEREvaluator()
        ref_text = RUNYANKOLE_ASR_EVAL_SET.metadata["samples"][0]["reference_text"]
        eval_res = evaluator.evaluate_pair(result, ref_text)
    ...
    def test_results_recorded_alongside_luganda_baseline(self):
        ...
        lug_comp = DummyASRComponent(default_text=LUGANDA_ASR_EVAL_SET.metadata["samples"][0]["reference_text"], language="lug")
        lug_res = lug_comp.run(Resource(id="lug_sample", kind=ResourceKind.SPEECH, language="lug", version="1.0.0", provenance={"consent_basis": "research"}))
        lug_evaluator = WEREvaluator()
        lug_eval = lug_evaluator.evaluate_pair(lug_res, LUGANDA_ASR_EVAL_SET.metadata["samples"][0]["reference_text"])
        lug_eval.provenance["dataset_version"] = "1.0.0"
        lug_eval.provenance["run_id"] = "lug_baseline_run_01"
        # Runyankole
        nyn_comp = RunyankoleASRComponent(default_text=RUNYANKOLE_ASR_EVAL_SET.metadata["samples"][0]["reference_text"], use_neural=False)
        nyn_res = nyn_comp.run(Resource(id="nyn_sample", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "research"}))
        nyn_evaluator = WEREvaluator()
        nyn_eval = nyn_evaluator.evaluate_pair(nyn_res, RUNYANKOLE_ASR_EVAL_SET.metadata["samples"][0]["reference_text"])
...
class TestM8RunyankolePipeline:
    def test_mock_config_loads_and_runs_end_to_end(self, tmp_path):
        ...
        input_res = Resource(id="nyn_test", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "research"})
    ...
    def test_mock_pipeline_evaluated_with_wer(self, tmp_path):
        ...
        pipeline = loader.load_file("configs/runyankole_mock_pipeline.yaml")
        executor = PipelineExecutor(pipeline)
        # Use a Result as input to control ASR fallback text? Instead run from Resource and then evaluate translation+tts doesn't affect WER.
        # For ASR-specific WER, we run the ASR stage alone
        asr_comp = RunyankoleASRComponent(default_text=RUNYANKOLE_ASR_EVAL_SET.metadata["samples"][0]["reference_text"], use_neural=False)
    ...
    def test_composed_only_from_registered_components(self):
        ...
        for name in pipeline.stage_names:
            assert (name, "1.0.0") in registry.list("component"), f"Component {name} not in registry"
    ...
    def test_pipeline_via_composition_without_core_change(self, tmp_path):
        ...
        executor = PipelineExecutor(pipeline)
        res = Resource(id="manual_nyn", kind=ResourceKind.SPEECH, language="nyn", version="1.0.0", provenance={"consent_basis": "manual_test"})
...
class TestM8ArchitecturalAudit:
    def test_no_core_registry_pipeline_changes(self):
        ...
        result = subprocess.run(
            ["git", "diff", "--name-only", "ec12c7a..HEAD", "--", "lingualdub/core", "lingualdub/registry", "lingualdub/pipeline"],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode != 0:
            result2 = subprocess.run(
                ["git", "status", "--porcelain", "--", "lingualdub/core", "lingualdub/registry", "lingualdub/pipeline"],
                capture_output=True,
                text=True,
            )
            assert result2.stdout.strip() == "", f"Core/registry/pipeline had uncommitted changes: {result2.stdout}"
        else:
            ...
            if diff_output:
                ...
                assert diff_output == "", f"M8 architectural audit failed: core/registry/pipeline changed:\n{diff_output}"
