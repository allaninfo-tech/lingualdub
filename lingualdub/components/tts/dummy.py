...
class DummyTTSComponent(TTSComponent):
            ...
            for idx, seg in enumerate(input.segments):
                ...
                if strategy == FittingStrategy.COMPRESS:
                    ...
                    for sub_idx, part in enumerate(parts):
                        ...
                        audio_path = (
                            self.output_dir
                            / f"tts_segment_{idx}_split_{sub_idx}_{self.version}.wav"
                        )
                        _write_dummy_wav(
                            audio_path,
                    ...
                    new_meta["unfit"] = True
                    warnings.append(
                        f"Segment #{idx} skipped (duration_ratio={ratio:.2f} exceeds threshold)"
                    )
                    out_segments.append(
                        Segment(
                            start=seg.start,
    ...
    def degrade(self, input: Result | Resource) -> Result:
        ...
        _write_dummy_wav(
            fallback_path, duration_sec=1.0, freq_hz=220.0, sample_rate=self.sample_rate
        )
